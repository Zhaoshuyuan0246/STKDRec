...
class GCL4SR(nn.Module):
    def __init__(self, args, global_graph):
        ...
        # self.item_embeddings = nn.Embedding(args.item_size, args.gnn_hidden_units, padding_idx=0)
        self.position_embeddings = nn.Embedding(args.maxlen, args.gnn_hidden_units)

        # sequence encoder
        # self.encoder_layer = nn.TransformerEncoderLayer(d_model=args.gnn_hidden_units,
        #                                                 nhead=args.num_attention_heads,
        #                                                 dim_feedforward=4 * args.gnn_hidden_units,
        #                                                 dropout=args.dropout_rate,
        #                                                 activation=args.hidden_act)
        # self.item_encoder = nn.TransformerEncoder(self.encoder_layer, num_layers=args.num_blocks)
        ...
        # fast run with mmd
        # self.w_g = nn.Linear(args.gnn_hidden_units, 1)
        # self.w_e = nn.Linear(args.gnn_hidden_units, 1)
    ...
    def gnn_encode(self, items, args):
        ...
        for (b_size, node_idx, adjs) in subgraph_loaders:
            if isinstance(adjs, list):
                # g_adjs_sparse = [adj_sparse.to(self.device) for adj_sparse in adjs_sparse]
                g_adjs.extend([adj.to(self.device) for adj in adjs])
            else:
                # g_adjs_sparse = adjs_sparse.to(self.device)
                g_adjs.append(adjs.to(self.device))
            n_idxs = node_idx.to(self.device)
            s_nodes = self.item_embeddings(n_idxs).squeeze()

        # g_hidden = self.global_gnn(s_nodes, g_adjs, self.global_graph.edge_attr)
    ...
    def final_att_net(self, seq_mask, hidden):
        ...
        for i in range(lens):
            ...
            # seq_hidden = torch.sum(current_hidden * current_mask.unsqueeze(-1), 1) / torch.sum(current_mask, 1).unsqueeze(-1)
            ...
            # seq_hidden = torch.sum(current_hidden * current_mask, -2) / torch.sum(current_mask, 1)
        ...
        # seq_hidden = seq_hidden.unsqueeze(-2).repeat(1, lens, 1)    #[128, 50, 64]
        # item_hidden = torch.matmul(torch.cat([pos_emb, hidden], -1), self.w_1)
        # item_hidden = torch.tanh(item_hidden)   #[128, 50, 64]
        # score = torch.sigmoid(self.linear_1(item_hidden) + self.linear_2(seq_hidden)) #[128, 50, 64]
        # att_score = torch.matmul(score, self.w_2) #[128, 50, 1]
        # att_score_masked = att_score * seq_mask    #[128, 50, 1]
        # output = torch.sum(att_score_masked * hidden, 1)    #[128, 64]
        # return output
    ...
    def forward(self, user_ids, inputs, pos_seqs, neg_seqs, args):
        ...
        if args.fus == 'kd':
            if args.loss_type == 'bce':
                ...
                test_item_emb = self.item_embeddings.weight[:self.args.item_size+1]
